    ...
    private static void play() {
        ...
        while (game.isRunning()) {
            for (CL_Agent a : _ar.getAgents()) {
                AgentThrad at = new AgentThrad(game, a);
                at.run();
...
//        System.out.println("**" + is_choosen);
    ...
    private static class AgentThrad implements Runnable{

        game_service _game;
        CL_Agent _a;

        public AgentThrad(game_service g, CL_Agent a) {
            _game = g;
            _a = a;
        }
        ...
        @Override
        public void run() {
            if (_paths.get(_a.getID()).isEmpty()) {
                createPath(_game, _a);
            }
            if (!_a.isMoving()) {
                nextMove(_game, _a);
